...
exports.parseRequirementScenariosFromText = parseRequirementScenariosFromText;
exports.parseRequirementScenarios = parseRequirementScenarios;
const fs = __importStar(require("node:fs"));
const safety_1 = require("../utils/safety");
const LABEL_KEYWORDS = 'Langkah|Steps?|Prekondisi|Precondition|Given|Hasil(?:\\s+yang\\s+Diharapkan)?|Expected(?:\\s+Result)?|Outcome|Input\\s+Data|Layer\\s+terdampak|Affected\\s+Layer';
function buildLabelRegex(keywords) {
    return new RegExp(`^\\*\\*(?:${keywords}):\\*\\*`, 'i');
}
const STEPS_LABEL = buildLabelRegex('Langkah|Steps?');
const RESULT_LABEL = buildLabelRegex('Hasil(?:\\s+yang\\s+Diharapkan)?|Expected(?:\\s+Result)?|Outcome');
...
function isManualScenario(rawHeading) {
    return /@manual/i.test(rawHeading);
}
function cleanScenarioName(rawHeading) {
    return rawHeading
        .replace(/\s*\(@(?:manual|success|failure|access-restriction)\)\s*/gi, ' ')
        .trim();
}
function extractScenarioType(rawHeading) {
    if (/@manual/i.test(rawHeading))
        return 'manual';
    if (/@failure/i.test(rawHeading))
        return 'failure';
    if (/@access-restriction/i.test(rawHeading))
        return 'access-restriction';
    if (/@success/i.test(rawHeading))
        return 'success';
    return 'general';
}
/**
 ...
 */
function parseRolesInScope(text) {
 ...
 */
function parseAccessExpectations(text) {
 ...
 */
function resolveScenarioRole(scenarioLines, rawName, rolesInScope) {
    for (const line of scenarioLines) {
        const m = line.match(/^\s*-\s+\*\*Role:\*\*\s*(.+)$/i) || line.match(/^\s*\*\*Role:\*\*\s*(.+)$/i);
        if (m) {
 ...
 */
function deriveAuthContext(text, role) {
 ...
 * skip a line.
 */
function parseBlock(lines, startIndex, opts) {
    const items = [];
    let text = '';
    let i = startIndex;
...
function transformParagraphLine(trimmed) {
    if (/^\*\*/.test(trimmed))
        return null; // skip stray label lines
    return trimmed.replace(/^[-*]\s+/, '').trim();
}
function stripLabel(trimmed, labelRegex) {
 ...
 *   3. 'general' fallback
 */
function resolveFeature(text, requirementPath) {
...
function resolveModule(text, requirementPath) {
    // 1. Explicit metadata field
 ...
 */
function parseTestId(scenarioLines) {
    for (const line of scenarioLines) {
 ...
 */
function parseScenarioPriority(scenarioLines) {
    for (const line of scenarioLines) {
 ...
 */
function parseAffectedLayer(scenarioLines) {
    const validLayers = new Set(['FE', 'BE', 'DB', 'API']);
    for (const line of scenarioLines) {
        ...
        return tokens.filter((t) => validLayers.has(t));
    }
    return [];
}
/**
 ...
 */
function parseInputDataBlock(lines, startIndex) {
    const inputData = {};
    let i = startIndex;
    while (i < lines.length) {
        const trimmed = lines[i].trim();
        if (BLOCK_TERMINATOR.test(trimmed) || HEADING_REGEX.test(trimmed))
            break;
 ...
 */
function parseExpectedResultFormatted(lines, startIndex) {
    const items = [];
    let i = startIndex;
    while (i < lines.length) {
        const trimmed = lines[i].trim();
...
function parseRequirementScenariosFromText(text) {
    const lines = text
    ...
    while (i < lines.length) {
        ...
        while (i < lines.length && !SCENARIO_HEADING_REGEX.test(lines[i]) && !/^##\s+/.test(lines[i])) {
            ...
            if (LAYER_LABEL.test(trimmed)) {
                ...
                while (i < lines.length) {
                    ...
                    if (BLOCK_TERMINATOR.test(t) || HEADING_REGEX.test(t))
                        break;
                    i += 1;
                }
                continue;
        ...
        if (name.length > 0 && steps.length > 0 && expectedResult.length > 0) {
            ...
            const resolvedTestId = testId.length > 0 ? testId : `TC-UNKNOWN-${String(scenarios.length + 1).padStart(3, '0')}`;
            const scenario = {
...
function parseRequirementScenarios(options) {
    ...
    if (scenarios.length === 0) {
        return {
            ...
            module: '-',
            feature: '-',
            message: 'No scenarios found. Use ### headings with **Langkah:**/**Steps:** and **Hasil:**/**Expected Result:** sections.',
...
//# sourceMappingURL=parse-requirement-scenarios.js.map