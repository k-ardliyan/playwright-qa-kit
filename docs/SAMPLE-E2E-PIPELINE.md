...
```bash
npx playwright-qa archive_report --runId=abc123-def456-ghi789 --reportPath=artifacts/reports/pipeline-report-<runId>.md
```
...
```bash
npx playwright-qa snapshot_page --url=/login --output=artifacts/selector-catalog/login/
```
...
# Archiving
npx playwright-qa archive_report --runId=<uuid> --reportPath=artifacts/reports/pipeline-report-<runId>.md
```
