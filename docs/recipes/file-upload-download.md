...
Committed samples live under `test-fixtures/`:

| Path                                      | Use                         |
| ----------------------------------------- | --------------------------- |
| `test-fixtures/images/sample.png`         | Image upload UI             |
| `test-fixtures/pdf/sample-text.pdf`       | PDF upload / content demo   |
| `test-fixtures/excel/sample-headers.xlsx` | Excel upload / headers demo |
| `test-fixtures/invalid/*`                 | Negative / spoofed cases    |
...
// or relative under test-fixtures/:
...
- fixture: test-fixtures/images/sample.png
...
| Headed pause / human picks a file          | Path under `test-fixtures/` in Input Data               |
| Absolute machine-only paths in specs       | `fixturePath(...)` or repo-relative `test-fixtures/...` |
| Assert product domain schema in helpers    | Scenario-owned tokens in Hasil (content recipe)         |

## Demo

```bash
npx playwright test src/tests/demo/demo-file-capabilities.spec.ts --project=demo
```
