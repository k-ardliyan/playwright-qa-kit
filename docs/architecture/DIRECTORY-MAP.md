...
| `src/contracts/`                | Typed QA Kit JSON contracts & hashing          | `index.ts`, `versions.ts`, `diagnostics.ts`, `hashing.ts`, `requirement-contract.ts`, `test-plan-contract.ts`, `traceability-contract.ts`, `mcp-result-contract.ts` |
