...
function parseTags(text, lines) {
    const tags = new Set();
    const tagLine = lines.find((line) => /^\s*tags?\s*:/i.test(line) || /^\s*-\s+\*\*Tags:\*\*/i.test(line));
    if (tagLine) {
        const raw = tagLine
            .replace(/^\s*-\s+\*\*Tags:\*\*/i, '')
            .replace(/^\s*tags?\s*:/i, '')
...
function extractSectionLines(lines, sectionPattern) {
    let inSection = false;
    const sectionLines = [];
...
function parseMetadataValue(line) {
...
function parseMetadata(lines, text) {
...
function parseAcceptanceCriteriaFromLines(lines) {
    const criteria = [];
...
function parseAcceptanceCriteria(lines) {
    const criteriaSection = extractSectionLines(lines, /(?:kriteria\s+penerimaan|acceptance\s+criteria)/i);
...
function normalizeRequirements(options) {
    ...
    if (input.requirementPath) {
        const resolved = (0, safety_1.resolveAllowedPath)(input.requirementPath, 'requirements', {
            mustExist: true,
            readOnly: true,
        });
        if (!resolved.ok) {
    ...
    const sizeError = (0, safety_1.assertRequirementsTextSize)(normalizedText);
    if (sizeError) {
        return {
            status: 'error',
    ...
    const scenarios = (0, parse_requirement_scenarios_1.parseRequirementScenariosFromText)(normalizedText);
    ...
    if (acceptanceCriteria.length === 0 && scenarios.length === 0) {
        return {
            ...
            error: {
                code: 'MALFORMED_INPUT',
                message: 'No acceptance criteria or test scenarios found. Use ## Kriteria Penerimaan bullets or ### scenario headings with Langkah/Hasil.',
            },
        };
    }
    const contract = {
    ...
    logger_1.logger.info('[playwright-qa] normalize_requirements is in maintenance mode; prefer compile_requirement (qa.requirement/v1).');
