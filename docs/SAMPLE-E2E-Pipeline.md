...
> **Purpose:** Complete working example of Playwright QA Kit pipeline from requirement to report  
...
```bash
npm run plan requirements/auth/login-flow.md
```
...
```bash
npm run generate specs/login-flow-test-plan.md
```
...
2. Applies generation rules from `generator.agent.md`:
   - Import from `@/fixtures/base.fixture` (never `@playwright/test`)
   - Create one spec file **per role**: `*-general.spec.ts`, `*-super-admin.spec.ts`
...
### File 1: `src/tests/login-flow-general.spec.ts`
...
test.describe('Login Flow - General', () => {
  test.use({
...
### File 2: `src/tests/login-flow-super-admin.spec.ts`
...
```bash
✅ src/tests/login-flow-general.spec.ts - PASSED (all tags matched)
✅ src/tests/login-flow-super-admin.spec.ts - PASSED (all tags matched)
...
```bash
npx playwright test src/tests/login-flow-general.spec.ts
npx playwright test src/tests/login-flow-super-admin.spec.ts
```
...
```json
{
  ...
  "byFile": [
    {
      "file": "src/tests/login-flow-general.spec.ts",
      "total": 2,
      "passed": 2,
      "failed": 0
    },
    {
      "file": "src/tests/login-flow-super-admin.spec.ts",
      "total": 1,
      "passed": 1,
      "failed": 0
    }
...
```bash
npx playwright-qa get_test_failures
```
...
```json
{
  "failures": [
    {
      "filePath": "src/tests/login-flow-general.spec.ts",
      "lineNumber": 38,
      ...
      "screenshotPath": "test-results/.../screenshot-1.png",
      "rootCause": "locator",
      "failureSource": "app"
    }
  ]
...
```bash
npm run validate
npx playwright test src/tests/login-flow-general.spec.ts --grep "TC-AUTH-002"
```
...
```bash
npx hermes report --runId=<pipeline-run-uuid>
```
...
`reports/pipeline-report-<runId>.json`
...
```json
{
  ...
  "mode": "role-aware",
  "rolesInScope": ["general", "super-admin"],
  "summaryByRole": {
    "general": { "passed": 2, "failed": 0, "skipped": 0 },
...
`reports/pipeline-report-<runId>.md`
...
| ------------------------------ | ------------------ | ----------- | --------- |
| SC-01: Valid credentials login | success            | general     | ✅ passed |
| SC-02: Invalid password        | failure            | general     | 🔧 healed |
...
```bash
npx playwright-qa archive_report --runId=abc123-def456-ghi789 --reportPath=reports/pipeline-report-<runId>.md
```

**Archive Created:** `reports/archive/abc123-def456-ghi789/`
Contains:
...
```bash
git add src/tests/login-flow*.spec.ts
git commit -m "feat(tests): add login flow tests for general & super-admin roles"
```
...
```markdown
## 2026-07-28 | src/tests/login-flow-general.spec.ts | Timeout waiting for error text | Locator mismatch | Changed to getByText().first()
```
...
```bash
npx playwright-qa snapshot_page --url=/login --output=selector-catalog/login/
```
...
# Reporting
npx hermes report --runId=<uuid>

# Archiving
npx playwright-qa archive_report --runId=<uuid> --reportPath=reports/pipeline-report-<runId>.md
```
