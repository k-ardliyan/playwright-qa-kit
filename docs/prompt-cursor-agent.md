...
Salin prompt di bawah ke Cursor (Agent mode). Ganti `requirements/nama-fitur.md` dengan path file Anda.

Pipeline mengikuti [CUSTOM-MCP.md — Agent pipeline checklist](../CUSTOM-MCP.md) dan [Orchestrator agent](../.github/agents/orchestrator.agent.md).
...
4. Generator: buat kode di src/tests/ dari test plan
...
2. Untuk halaman baru: browser_navigate + browser_snapshot (playwright) sebelum tulis selector
3. Tulis file di src/tests/ (kebab-case .spec.ts)
...
2. Perbaiki file spec yang gagal (gunakan tracePath/screenshotPath jika ada)
