...
npx playwright test -c examples/erpku/playwright.config.ts --project=setup --workers=1
# OTP/CAPTCHA (browser terlihat):
npx playwright test -c examples/erpku/playwright.config.ts --project=setup --workers=1 --headed
```
...
- **`src/`** — generic template core.
- **`examples/erpku/`** — ERPKU-specific pages, fixtures, and tests. CI E2E runs this adapter when secrets are configured.
...
- **AI Generator does not write new specs here** — pipeline output stays in template core `tests/` (see [docs/GUIDE.md](../../docs/GUIDE.md))
